...
APP_VERSION = "v15.5.49"
APP_DOCUMENTED_REVISIONS = 171
APP_VERSION_NOTE = f"{APP_VERSION} · ≥{APP_DOCUMENTED_REVISIONS} iteraciones únicas de desarrollo documentadas"
...
def _autonomic_classify_instantaneous_v15546(
    V_abs, S_abs, L_abs, C_abs, R_abs,
    hfdom=np.nan, lfdom=np.nan, lfhf=np.nan,
    idx_amp=np.nan, oscillatory_current=None, context_scores=None,
    quality=0.75
):
    """v15.5.46 · Topología instantánea fuzzy/adaptativa recalibrada.
    ...
    if sum(np.isfinite(x) for x in vals)<3 or not(np.isfinite(V_abs) and np.isfinite(S_abs)):
        return {
            ...
            'Score_Reciprocidad':np.nan,'Score_Reciprocidad_bruto':np.nan,
            'Score_Coinhibicion':np.nan,'Score_Coinhibicion_bruto':np.nan,
            'Score_Tendencia_Coinhibicion':np.nan,'Score_Tendencia_Coinhibicion_bruto':np.nan,
            'Tendencia_Coinhibicion_permitida':False,
            'Score_Desacoplamiento':np.nan,'Score_Desacoplamiento_bruto':np.nan,
            'Score_Incoherencia':np.nan,'Score_Intermedio':np.nan,
            'Desacoplamiento_permitido':False,'Incoherencia_detectada':False,
            'Coactivacion_permitida':False,'Reciprocidad_permitida':False,'Coinhibicion_permitida':False,
    ...
    # v15.5.46 · Corrección de calibración:
    # L/C/R pueden APOYAR una coactivación, pero no transformar dos ramas V/S
    # desplazadas conjuntamente hacia negativo en "coactivación".
    both_negative=bool(V_abs<0 and S_abs<0)
    both_positive=bool(V_abs>0 and S_abs>0)
    one_high_other_preserved_nonnegative=bool(
        (mv['high']>=0.45 and ms['preserved']>=0.45 and S_abs>=-0.02)
        or (ms['high']>=0.45 and mv['preserved']>=0.45 and V_abs>=-0.02)
    )
    coact_core=float(np.clip(max(
        joint_high,
        joint_pres*(n_coact_supports/4.0) if (both_positive or one_high_other_preserved_nonnegative) else 0.0
    ),0,1))
    coactivation_allowed=bool(
        not both_negative
        and (
            joint_high>=0.38
            or (
                joint_pres>=0.50
                and n_coact_supports>=2
                and abs_gap<=gray*1.25
                and (both_positive or one_high_other_preserved_nonnegative)
            )
        )
    )

    # Gradiente negativo conjunto dentro de la zona preservada:
    # no es coinhibición franca, pero tampoco coactivación.
    negative_joint_tendency=bool(
        both_negative
        and max(abs(V_abs),abs(S_abs))<=gray*1.20
        and joint_low>=0.30
        and not technical_discordance
    )
    ...
    coinhib_core=float(np.clip(0.75*joint_low+0.25*lcr_reduced,0,1))
    # Coinhibición franca exige reducción conjunta suficiente y salida real de la zona
    # preservada en al menos una rama (o pertenencia baja conjunta muy alta).
    coinhibition_allowed=bool(
        joint_low>=0.42
        and (min(V_abs,S_abs)<=-gray or joint_low>=0.55)
        and not multiband_strong
        and not technical_discordance
    )

    # Tendencia a coinhibición: estado aún preservado, pero ambas ramas se desplazan
    # de manera congruente hacia negativo. Se mantiene como familia separada.
    score_coinhib_tendency_raw=float(np.clip(
        0.50 + 0.30*joint_low + 0.20*lcr_reduced,
        0,1
    ))
    ...
    score_recip=score_recip_raw if reciprocity_allowed else -1.0
    score_coinhib=score_coinhib_raw if coinhibition_allowed else -1.0
    score_coinhib_tendency=score_coinhib_tendency_raw if negative_joint_tendency and not coinhibition_allowed else -1.0
    score_uncoupled=score_uncoupled_raw if unilateral_decoupling else -1.0
    score_incoherence=incoherence_raw if technical_discordance else -1.0

    max_phys=max([v for v in [score_coact,score_recip,score_coinhib,score_coinhib_tendency,score_uncoupled] if v>=0] or [0.0])
    ambiguity=float(np.clip(1.0-max_phys,0,1))
    score_intermediate=float(np.clip(0.55*ambiguity+0.45*min(mv['preserved'],ms['preserved'])*(1.0 if n_coact_supports<2 else 0.35),0,1))
    if technical_discordance:
        score_intermediate=float(np.clip(score_intermediate+0.20,0,1))
    ...
    scores={
        ...
        'Reciprocidad':score_recip,
        'Coinhibición':score_coinhib,
        'Patrón preservado con tendencia a coinhibición':score_coinhib_tendency,
        'Desacoplamiento fisiológico unilateral':score_uncoupled,
        'Patrón complejo/incoherente':score_incoherence,
        'Patrón intermedio/no concluyente':score_intermediate,
    }
    ...
    elif topology=='Patrón preservado con tendencia a coinhibición': qualifier='V/S preservados con desplazamiento conjunto negativo'
    elif margin<0.07: qualifier='limítrofe'
    elif margin<0.14: qualifier='moderadamente definida'
    else: qualifier='definida'
    if topology=='Coactivación' and lcr_functional>=0.65: qualifier='funcional por convergencia multidimensional'
    ...
    gate_note=(f'Fuzzy Q={q:.2f}, gris=±{gray:.2f}. '
        f'Coactivación={"sí" if coactivation_allowed else "no"} ({n_coact_supports}/4 apoyos); '
        f'Reciprocidad={"sí" if reciprocity_allowed else "no"}; Coinhibición={"sí" if coinhibition_allowed else "no"}; '
        f'Tendencia coinhibición={"sí" if negative_joint_tendency else "no"}; '
        f'Desacoplamiento unilateral={"sí" if unilateral_decoupling else "no"}; '
        f'Incoherencia={"sí" if technical_discordance else "no"}.')
    ...
    return {
        ...
        'Nota_convergencia_actual':('Topología fuzzy/adaptativa. Desacoplamiento fisiológico se separa de incoherencia de señal. '
            f'Scores → Coact {score_coact:.2f}, Recip {score_recip:.2f}, Coinhib {score_coinhib:.2f}, '
            f'TendenciaCoinhib {score_coinhib_tendency:.2f}, Desacoplamiento {score_uncoupled:.2f}, '
            f'Incoherencia {score_incoherence:.2f}, Intermedio {score_intermediate:.2f}. {gate_note} {osc_note}'),
        'Score_Coactivacion':float(score_coact),'Score_Coactivacion_bruto':float(score_coact_raw),
        'Score_Reciprocidad':float(score_recip),'Score_Reciprocidad_bruto':float(score_recip_raw),
        'Score_Coinhibicion':float(score_coinhib),'Score_Coinhibicion_bruto':float(score_coinhib_raw),
        'Score_Tendencia_Coinhibicion':float(score_coinhib_tendency),
        'Score_Tendencia_Coinhibicion_bruto':float(score_coinhib_tendency_raw),
        'Tendencia_Coinhibicion_permitida':bool(negative_joint_tendency),
        'Score_Desacoplamiento':float(score_uncoupled),'Score_Desacoplamiento_bruto':float(score_uncoupled_raw),
        'Score_Incoherencia':float(score_incoherence),'Score_Intermedio':float(score_intermediate),
        'Desacoplamiento_permitido':bool(unilateral_decoupling),'Incoherencia_detectada':bool(technical_discordance),
        'Coactivacion_permitida':bool(coactivation_allowed),'Reciprocidad_permitida':bool(reciprocity_allowed),
...
def _autonomic_classify_instantaneous_v15547(
    V_abs, S_abs, L_abs, C_abs, R_abs,
    hfdom=np.nan, lfdom=np.nan, lfhf=np.nan,
    idx_amp=np.nan, oscillatory_current=None, context_scores=None,
    quality=0.75
):
    """v15.5.47 · Topología instantánea fuzzy/adaptativa con gradiente continuo de coinhibición.
    ...
    if sum(np.isfinite(x) for x in vals)<3 or not(np.isfinite(V_abs) and np.isfinite(S_abs)):
        return {
            ...
            'Score_Tendencia_Coinhibicion':np.nan,'Score_Tendencia_Coinhibicion_bruto':np.nan,
            'Tendencia_Coinhibicion_permitida':False,
            'Profundidad_negativa_media_VS':np.nan,
            'Profundidad_negativa_max_VS':np.nan,
            'Score_Desacoplamiento':np.nan,'Score_Desacoplamiento_bruto':np.nan,
            'Score_Incoherencia':np.nan,'Score_Intermedio':np.nan,
            'Desacoplamiento_permitido':False,'Incoherencia_detectada':False,
            'Coactivacion_permitida':False,'Reciprocidad_permitida':False,'Coinhibicion_permitida':False,
    ...
    # v15.5.47 · Gradiente continuo de tendencia a coinhibición.
    # El criterio anterior dependía demasiado de joint_low y podía dejar fuera
    # pares como V=-0.05/S=-0.13 aun estando ambas ramas dirigidas hacia negativo.
    #
    # Se usa profundidad negativa RELATIVA a la zona gris:
    # - evita que -0.01/-0.02 se etiquete como tendencia;
    # - permite detectar una dirección conjunta negativa clínicamente legible
    #   antes de llegar a coinhibición franca.
    v_neg_depth=float(np.clip((-V_abs)/gray,0,1.5)) if gray>0 else 0.0
    s_neg_depth=float(np.clip((-S_abs)/gray,0,1.5)) if gray>0 else 0.0
    mean_negative_depth=float((v_neg_depth+s_neg_depth)/2.0)
    max_negative_depth=float(max(v_neg_depth,s_neg_depth))
    negative_joint_tendency=bool(
        both_negative
        and mean_negative_depth>=0.30
        and max_negative_depth>=0.45
        and max(abs(V_abs),abs(S_abs))<=gray*1.20
        and not technical_discordance
    )
    ...
    score_coinhib_tendency_raw=float(np.clip(
        0.52
        + 0.22*np.clip(mean_negative_depth,0,1)
        + 0.14*np.clip(max_negative_depth,0,1)
        + 0.07*joint_low
        + 0.05*lcr_reduced,
        0,1
    ))
    ...
    gate_note=(f'Fuzzy Q={q:.2f}, gris=±{gray:.2f}. '
        ...
        f'Reciprocidad={"sí" if reciprocity_allowed else "no"}; Coinhibición={"sí" if coinhibition_allowed else "no"}; '
        f'Tendencia coinhibición={"sí" if negative_joint_tendency else "no"} '
        f'(profundidad media={mean_negative_depth:.2f}, máxima={max_negative_depth:.2f}); '
        f'Desacoplamiento unilateral={"sí" if unilateral_decoupling else "no"}; '
        f'Incoherencia={"sí" if technical_discordance else "no"}.')
    ...
    return {
        ...
        'Score_Tendencia_Coinhibicion_bruto':float(score_coinhib_tendency_raw),
        'Tendencia_Coinhibicion_permitida':bool(negative_joint_tendency),
        'Profundidad_negativa_media_VS':float(mean_negative_depth),
        'Profundidad_negativa_max_VS':float(max_negative_depth),
        'Score_Desacoplamiento':float(score_uncoupled),'Score_Desacoplamiento_bruto':float(score_uncoupled_raw),
        'Score_Incoherencia':float(score_incoherence),'Score_Intermedio':float(score_intermediate),
        'Desacoplamiento_permitido':bool(unilateral_decoupling),'Incoherencia_detectada':bool(technical_discordance),
        'Coactivacion_permitida':bool(coactivation_allowed),'Reciprocidad_permitida':bool(reciprocity_allowed),
...
def _autonomic_classify_instantaneous_v15548(
    V_abs, S_abs, L_abs, C_abs, R_abs,
    hfdom=np.nan, lfdom=np.nan, lfhf=np.nan,
    idx_amp=np.nan, oscillatory_current=None, context_scores=None,
    quality=0.75
):
    """v15.5.48 · Topología instantánea con estado de referencia/preservado explícito.
    ...
    if sum(np.isfinite(x) for x in vals)<3 or not(np.isfinite(V_abs) and np.isfinite(S_abs)):
        return {
            ...
            'Profundidad_negativa_max_VS':np.nan,
            'Score_Desacoplamiento':np.nan,'Score_Desacoplamiento_bruto':np.nan,
            'Score_Incoherencia':np.nan,'Score_Preservado':np.nan,'Score_Intermedio':np.nan,
            'Desacoplamiento_permitido':False,'Incoherencia_detectada':False,
            'Coactivacion_permitida':False,'Reciprocidad_permitida':False,'Coinhibicion_permitida':False,
            'Estado_V_actual':_state(V_abs),'Estado_S_actual':_state(S_abs),
            'Pertenencia_V':_memberships(V_abs),'Pertenencia_S':_memberships(S_abs),
    ...
    # v15.5.48 · Estado de referencia/preservado explícito.
    # El punto (V,S)=(0,0) es el centro de REFERENCIA CONTEXTUAL, no "actividad autonómica cero".
    # Una posición próxima al centro puede ser una conclusión válida (estado preservado) y no debe
    # caer automáticamente en "intermedio/no concluyente".
    max_abs_vs=float(max(abs(V_abs),abs(S_abs)))
    radius_vs=float(np.sqrt((V_abs/gray)**2 + (S_abs/gray)**2)/np.sqrt(2.0)) if gray>0 else np.inf
    central_reference=bool(max_abs_vs<=gray*0.35)
    preserved_zone=bool(
        max_abs_vs<=gray*1.15
        and not negative_joint_tendency
        and not technical_discordance
        and not unilateral_decoupling
    )
    # La puntuación premia proximidad al centro y pertenencia preservada de ambas ramas.
    score_preserved_raw=float(np.clip(
        0.46
        + 0.34*np.clip(1.0-radius_vs/1.15,0,1)
        + 0.20*joint_pres,
        0,1
    ))
    ...
    score_uncoupled=score_uncoupled_raw if unilateral_decoupling else -1.0
    score_incoherence=incoherence_raw if technical_discordance else -1.0
    score_preserved=score_preserved_raw if preserved_zone else -1.0

    max_phys=max([v for v in [score_coact,score_recip,score_coinhib,score_coinhib_tendency,score_uncoupled,score_preserved] if v>=0] or [0.0])
    ambiguity=float(np.clip(1.0-max_phys,0,1))
    score_intermediate=float(np.clip(0.55*ambiguity+0.45*min(mv['preserved'],ms['preserved'])*(1.0 if n_coact_supports<2 else 0.35),0,1))
    if preserved_zone:
        # "Intermedio" queda reservado para ambigüedad real, no para ausencia de desplazamiento.
        score_intermediate=float(np.clip(score_intermediate*0.42,0,1))
    if technical_discordance:
        score_intermediate=float(np.clip(score_intermediate+0.20,0,1))
    ...
    scores={
        ...
        'Patrón preservado con tendencia a coinhibición':score_coinhib_tendency,
        'Desacoplamiento fisiológico unilateral':score_uncoupled,
        'Estado autonómico preservado':score_preserved,
        'Patrón complejo/incoherente':score_incoherence,
        'Patrón intermedio/no concluyente':score_intermediate,
    }
    ordered=sorted(scores.items(),key=lambda kv:kv[1],reverse=True)
    ...
    # Evitar asignaciones fisiológicas débiles; incoherencia sí puede dominar directamente.
    if topology not in ('Patrón complejo/incoherente','Patrón intermedio/no concluyente','Estado autonómico preservado') and best<0.38:
        topology='Patrón intermedio/no concluyente'; best=score_intermediate
    margin=best-second
    if topology=='Patrón complejo/incoherente': qualifier='requiere revisión de señal/métodos'
    elif topology=='Patrón intermedio/no concluyente': qualifier='señales limítrofes o competitivas sin una familia dominante'
    elif topology=='Estado autonómico preservado':
        qualifier='referencia contextual' if central_reference else 'sin desplazamiento autonómico significativo'
    elif topology=='Patrón preservado con tendencia a coinhibición': qualifier='V/S preservados con desplazamiento conjunto negativo'
    elif margin<0.07: qualifier='limítrofe'
    elif margin<0.14: qualifier='moderadamente definida'
    else: qualifier='definida'
    ...
    # En estado preservado el predominio describe una RELACIÓN entre ramas, no una activación intensa.
    # Si V y S están demasiado próximos, el espectro no debe fabricar por sí solo un "predominio":
    # el resultado principal es referencia/preservación. Sólo una separación V-S suficiente se expresa
    # como predominio relativo ligero.
    if topology=='Estado autonómico preservado':
        if abs_gap < pred_thr:
            predominance='Sin predominio claro'
            predominance_source='V/S próximos dentro de estado preservado'
        elif predominance.startswith('Predominio '):
            predominance='Ligero predominio relativo ' + predominance.replace('Predominio ','',1)
            predominance_source=predominance_source + ' · desplazamiento global pequeño'
    ...
    gate_note=(f'Fuzzy Q={q:.2f}, gris=±{gray:.2f}. '
        ...
        f'(profundidad media={mean_negative_depth:.2f}, máxima={max_negative_depth:.2f}); '
        f'Desacoplamiento unilateral={"sí" if unilateral_decoupling else "no"}; '
        f'Preservado={"sí" if preserved_zone else "no"} (radio V/S={radius_vs:.2f}); '
        f'Incoherencia={"sí" if technical_discordance else "no"}.')
    ...
    return {
        ...
        'Nota_convergencia_actual':('Topología fuzzy/adaptativa. Desacoplamiento fisiológico se separa de incoherencia de señal. '
            f'Scores → Coact {score_coact:.2f}, Recip {score_recip:.2f}, Coinhib {score_coinhib:.2f}, '
            f'TendenciaCoinhib {score_coinhib_tendency:.2f}, Desacoplamiento {score_uncoupled:.2f}, '
            f'Preservado {score_preserved:.2f}, Incoherencia {score_incoherence:.2f}, '
            f'Intermedio {score_intermediate:.2f}. {gate_note} {osc_note}'),
        'Score_Coactivacion':float(score_coact),'Score_Coactivacion_bruto':float(score_coact_raw),
        'Score_Reciprocidad':float(score_recip),'Score_Reciprocidad_bruto':float(score_recip_raw),
        'Score_Coinhibicion':float(score_coinhib),'Score_Coinhibicion_bruto':float(score_coinhib_raw),
        'Score_Tendencia_Coinhibicion':float(score_coinhib_tendency),
        ...
        'Profundidad_negativa_max_VS':float(max_negative_depth),
        'Score_Desacoplamiento':float(score_uncoupled),'Score_Desacoplamiento_bruto':float(score_uncoupled_raw),
        'Score_Incoherencia':float(score_incoherence),'Score_Preservado':float(score_preserved),
        'Score_Intermedio':float(score_intermediate),
        'Desacoplamiento_permitido':bool(unilateral_decoupling),
        'Estado_preservado_permitido':bool(preserved_zone),
        'Referencia_contextual_central':bool(central_reference),
        'Radio_VS_zona_gris':float(radius_vs),
        'Incoherencia_detectada':bool(technical_discordance),
        'Coactivacion_permitida':bool(coactivation_allowed),'Reciprocidad_permitida':bool(reciprocity_allowed),
        'Coinhibicion_permitida':bool(coinhibition_allowed),'Estado_V_actual':_state(V_abs),'Estado_S_actual':_state(S_abs),
        'Pertenencia_V':mv,'Pertenencia_S':ms,'Umbral_gris_actual':gray,
        'Contribucion_VS_pct':70,'Contribucion_LCR_pct':30,
...
def _autonomic_dynamic_integrity_v15544(row, quality=np.nan, incoherent=False, age_years=None):
    """IID 0-100: caracteriza el desacoplamiento; no redefine la topología V/S."""
    def n(k):
        try:
            v=float(pd.to_numeric(row.get(k,np.nan),errors='coerce'))
            return v if np.isfinite(v) else np.nan
        except Exception: return np.nan
    def band(x,lo,hi,shoulder):
        if not np.isfinite(x): return np.nan
        if lo<=x<=hi: return 1.0
        d=lo-x if x<lo else x-hi
        return float(np.clip(1.0-d/max(shoulder,1e-9),0,1))
    def mf(v):
        v=[float(x) for x in v if np.isfinite(x)]
        return float(np.mean(v)) if v else np.nan

    fractal=mf([band(n('DFA_alpha1'),.85,1.15,.45),
                band(n('DFA_alpha2'),.90,1.10,.45),
                band(n('Hurst'),.65,.85,.30)])

    age=pd.to_numeric(age_years,errors='coerce')
    if np.isfinite(age):
        slo,shi=(1.30,2.10) if age<40 else ((1.10,1.80) if age<60 else (.80,1.50))
    else: slo,shi=.80,2.10
    ep=[band(n('SampEn'),slo,shi,.75),band(n('DispEn'),1.80,2.60,1.00)]
    for prefix in ('MSE','MDE'):
        a=np.asarray([n(f'{prefix}{i}') for i in range(1,21)],dtype=float)
        a=a[np.isfinite(a)&(a>=0)]
        if len(a)>=5:
            positive=float(np.mean(a>.05))
            cv=float(np.std(a)/(abs(np.mean(a))+1e-9))
            ep.append(float(np.clip(.65*positive+.35*(1-np.clip((cv-.80)/1.50,0,1)),0,1)))
    entropy=mf(ep)

    lle=n('Lyapunov_LLE')
    lle_s=float(np.clip(lle/.20,0,1)) if np.isfinite(lle) and lle>0 else (.25 if np.isfinite(lle) else np.nan)
    attractor=mf([band(n('D2'),2.50,4.50,2.00),lle_s])

    det=n('DET'); lmax=n('Lmax'); sh=n('ShanEn')
    recurrence=mf([band(det,60,85,20),band(lmax,15,50,60),
                   float(np.clip(sh/3,0,1)) if np.isfinite(sh) else np.nan])

    sw=n('HVG_graph_score_small_world'); sf=n('HVG_graph_score_scale_free')
    hv=[float(np.clip(x/100,0,1)) for x in (sw,sf) if np.isfinite(x)]
    hvg=max(hv) if hv else np.nan

    blocks={'Fractalidad':fractal,'Entropía multiescala':entropy,'Atractor/geometría':attractor,
            'Recurrencia':recurrence,'HVG/red':hvg}
    weights={'Fractalidad':.24,'Entropía multiescala':.24,'Atractor/geometría':.18,
             'Recurrencia':.18,'HVG/red':.16}
    finite={k:v for k,v in blocks.items() if np.isfinite(v)}
    coverage=len(finite)/5
    if finite:
        ws=sum(weights[k] for k in finite)
        iid=100*sum(weights[k]*finite[k] for k in finite)/ws
    else: iid=np.nan
    q=float(np.clip(quality,0,1)) if np.isfinite(quality) else .70
    confidence=float(np.clip(100*(.55*coverage+.45*q),0,100))

    rigid=stoch=0
    dfa1=n('DFA_alpha1'); samp=n('SampEn')
    if np.isfinite(dfa1):
        rigid+=int(dfa1>=1.35); stoch+=int(dfa1<=.65)
    rigid+=int(np.isfinite(det) and det>=90)
    rigid+=int(np.isfinite(lmax) and lmax>70)
    if np.isfinite(samp):
        rigid+=int(samp<slo*.70); stoch+=int(samp>shi*1.35)

    if incoherent or q<.45 or coverage<.40:
        cls="Indeterminado por inestabilidad/incoherencia"
        detail="La calidad o convergencia entre métodos no permite atribuir un significado fisiológico estable."
    elif iid>=68:
        cls="Adaptativo/funcional"
        detail="Conserva organización fractal, complejidad y estructura dinámica suficientes."
    elif iid<=42:
        cls="Compatible con pérdida de complejidad"
        if rigid>=2 and rigid>stoch:
            detail="Fenotipo de rigidificación/orden excesivo; no equivale por sí solo a diagnóstico patológico."
        elif stoch>=2 and stoch>rigid:
            detail="Fenotipo de desorganización/estocasticidad excesiva; debe excluirse señal inestable o artefacto."
        else:
            detail="Reducción convergente de integridad dinámica; requiere correlación clínica y no constituye diagnóstico por sí sola."
    else:
        cls="Indeterminado/intermedio"
        detail="Integridad dinámica mixta, sin evidencia suficiente para clasificarla como claramente adaptativa o reducida."

    return {'IID':float(iid) if np.isfinite(iid) else np.nan,'IID_confianza':confidence,'IID_cobertura':coverage,
            'Caracterizacion_desacoplamiento':cls,'Detalle_caracterizacion_desacoplamiento':detail,
            'IID_Fractalidad':fractal*100 if np.isfinite(fractal) else np.nan,
            'IID_Entropia_multiescala':entropy*100 if np.isfinite(entropy) else np.nan,
            'IID_Atractor_geometria':attractor*100 if np.isfinite(attractor) else np.nan,
            'IID_Recurrencia':recurrence*100 if np.isfinite(recurrence) else np.nan,
            'IID_HVG_red':hvg*100 if np.isfinite(hvg) else np.nan,
            'IID_fenotipo_rigidez_n':rigid,'IID_fenotipo_desorganizacion_n':stoch}
...
def _autonomic_dual_dashboard_v15543(
    ...
    fig.add_annotation(
        x=0,y=0,text="Zona preservada / referencia<br>/ intermedia",
        ...
        go.Scatter(
            ...
            marker=dict(size=17,color="#2C3E50",symbol="cross",
                line=dict(width=4,color=(
                    "#27AE60" if "Adaptativo/funcional" in str(dynamic_class)
                    else "#C0392B" if "pérdida de complejidad" in str(dynamic_class)
                    else "#F39C12" if "Indeterminado" in str(dynamic_class)
                    else "white"
                ))),
            hovertemplate=(
    ...
    if dynamic_class:
        subtitle += (f" · IID {iid:.0f}/100 · {dynamic_class}" if np.isfinite(iid) else f" · {dynamic_class}")
...
def autonomic_control_assessment(df, row, record_data=None):
    ...
    instant = _autonomic_classify_instantaneous_v15548(
        V_abs, S_abs, L_abs, C_abs, R_abs,
        hfdom=get('HF_DOM_PCT'),
        lfdom=get('LF_DOM_PCT'),
        lfhf=get('LF_HF'),
    ...
    _iid_v15544 = _autonomic_dynamic_integrity_v15544(
        row, quality=Q,
        incoherent=bool(instant.get('Incoherencia_detectada', False)),
        age_years=_profile_v15528.get('age_years')
    )
    if instant.get('topologia') != 'Desacoplamiento fisiológico unilateral':
        _iid_v15544['Caracterizacion_desacoplamiento']='No aplica'
        _iid_v15544['Detalle_caracterizacion_desacoplamiento']=(
            'El IID se conserva como descriptor dinámico, pero sólo caracteriza clínicamente '
            'el desacoplamiento cuando la topología V/S es unilateral.'
        )
    ...
    return {
        ...
        'Score_Tendencia_Coinhibicion_actual': instant.get('Score_Tendencia_Coinhibicion', np.nan),
        'Score_Tendencia_Coinhibicion_bruto_actual': instant.get('Score_Tendencia_Coinhibicion_bruto', np.nan),
        'Tendencia_Coinhibicion_permitida': instant.get('Tendencia_Coinhibicion_permitida', False),
        'Profundidad_negativa_media_VS': instant.get('Profundidad_negativa_media_VS', np.nan),
        'Profundidad_negativa_max_VS': instant.get('Profundidad_negativa_max_VS', np.nan),
        ...
        'Score_Preservado_actual': instant.get('Score_Preservado', np.nan),
        'Estado_preservado_permitido': instant.get('Estado_preservado_permitido', False),
        'Referencia_contextual_central': instant.get('Referencia_contextual_central', False),
        'Radio_VS_zona_gris': instant.get('Radio_VS_zona_gris', np.nan),
        ...
        'Caracterizacion_desacoplamiento': _iid_v15544.get('Caracterizacion_desacoplamiento','No aplica'),
        'Detalle_caracterizacion_desacoplamiento': _iid_v15544.get('Detalle_caracterizacion_desacoplamiento',''),
        'IID': _iid_v15544.get('IID', np.nan),
        'IID_confianza': _iid_v15544.get('IID_confianza', np.nan),
        'IID_cobertura': _iid_v15544.get('IID_cobertura', np.nan),
        'IID_Fractalidad': _iid_v15544.get('IID_Fractalidad', np.nan),
        'IID_Entropia_multiescala': _iid_v15544.get('IID_Entropia_multiescala', np.nan),
        'IID_Atractor_geometria': _iid_v15544.get('IID_Atractor_geometria', np.nan),
        'IID_Recurrencia': _iid_v15544.get('IID_Recurrencia', np.nan),
        'IID_HVG_red': _iid_v15544.get('IID_HVG_red', np.nan),
...
if _history_source_v1538 == "Sesión actual" and record_data:
    try:
        _bundle_sig_payload_v15545 = (
            tuple(sorted(
                str(_data.get('record_key') or _raw_record_key(_rec, _data.get('filename', _rec)))
                for _rec, _data in record_data.items()
            )),
            tuple(map(str, active_phases)),
            bool(use_independent),
        )
        _bundle_sig_v15545 = hashlib.sha256(repr(_bundle_sig_payload_v15545).encode("utf-8")).hexdigest()
        if st.session_state.get("_bundle_saved_signature_v15545") != _bundle_sig_v15545:
            ...
            for _pat, _keys in _bundle_groups_v1539.items():
                save_patient_analysis_bundle(_pat, _keys, config={
                    'last_session_version': '15.5.45',
                    'active_phases': list(active_phases),
                    'updated_from_session': True,
                }, include_all_snapshotted=True)
            st.session_state["_bundle_saved_signature_v15545"] = _bundle_sig_v15545
...
_perf_aggregate_payload_v15545 = {
    "records": tuple(map(str, records)),
    "active_phases": tuple(map(str, active_phases)),
    "use_independent": bool(use_independent),
    "min_rr": int(min_rr),
    "include_rqa": bool(include_rqa),
    "include_hvg": bool(include_hvg),
    "mse_zero_policy": st.session_state.get("mse_zero_policy", "nan"),
    "sampen_theiler_window": st.session_state.get("sampen_theiler_window", 0),
    "mse_radius_mode": st.session_state.get("mse_radius_mode", "fixed_entropy_sd"),
    "windows": {
        str(_r): get_record_windows(global_windows_safe, record_windows_safe, _r, use_independent)
        for _r in records
    },
}
_perf_aggregate_sig_v15545 = hashlib.sha256(
    repr(_perf_aggregate_payload_v15545).encode("utf-8")
).hexdigest()
_perf_aggregate_cache_v15545 = st.session_state.get("_perf_aggregate_cache_v15545", {})

if _perf_aggregate_cache_v15545.get("signature") == _perf_aggregate_sig_v15545:
    long_df = _perf_aggregate_cache_v15545.get("long_df", pd.DataFrame()).copy()
    indices_df = _perf_aggregate_cache_v15545.get("indices_df", pd.DataFrame()).copy()
else:
    long_df = build_long(records_results)
    indices_df = build_physiological_indices(long_df)
    if indices_df is not None and not indices_df.empty:
        indices_df['Artefactos_pct'] = indices_df['Registro'].map(
            lambda _r: float((record_data.get(_r,{}).get('artifact_info') or {}).get('percent_artifacts',0.0))
            if _r in record_data else np.nan
        )
    st.session_state["_perf_aggregate_cache_v15545"] = {
        "signature": _perf_aggregate_sig_v15545,
        "long_df": long_df.copy(),
        "indices_df": indices_df.copy(),
    }

_perf_patient_cache_v15545 = st.session_state.get("_perf_patient_frames_v15545", {})
if _perf_patient_cache_v15545.get("signature") != _perf_aggregate_sig_v15545:
    _patient_long_v15545, _patient_indices_v15545 = {}, {}
    if isinstance(long_df, pd.DataFrame) and not long_df.empty and "Registro" in long_df.columns:
        _tmp_long_v15545 = long_df.copy()
        if "Paciente_ID" not in _tmp_long_v15545.columns:
            _tmp_long_v15545["Paciente_ID"] = _tmp_long_v15545["Registro"].astype(str).map(infer_patient_id)
        _tmp_long_v15545["Paciente_ID"] = _tmp_long_v15545["Paciente_ID"].astype(str).map(normalize_patient_id)
        for _pid, _g in _tmp_long_v15545.groupby("Paciente_ID", sort=False):
            _patient_long_v15545[str(_pid)] = _g.copy()
    if isinstance(indices_df, pd.DataFrame) and not indices_df.empty and "Registro" in indices_df.columns:
        _tmp_idx_v15545 = indices_df.copy()
        if "Paciente_ID" not in _tmp_idx_v15545.columns:
            _tmp_idx_v15545["Paciente_ID"] = _tmp_idx_v15545["Registro"].astype(str).map(infer_patient_id)
        _tmp_idx_v15545["Paciente_ID"] = _tmp_idx_v15545["Paciente_ID"].astype(str).map(normalize_patient_id)
        for _pid, _g in _tmp_idx_v15545.groupby("Paciente_ID", sort=False):
            _patient_indices_v15545[str(_pid)] = _g.copy()
    _perf_patient_cache_v15545 = {
        "signature": _perf_aggregate_sig_v15545,
        "long": _patient_long_v15545,
        "indices": _patient_indices_v15545,
    }
    st.session_state["_perf_patient_frames_v15545"] = _perf_patient_cache_v15545
...
# ============================================================
# v15.5.45 · PERFORMANCE LARGE COHORTS
# ============================================================
_PERF_RECORD_COUNT_V15545 = len(records)
_PERF_LARGE_COHORT_V15545 = _PERF_RECORD_COUNT_V15545 > 40
_PERF_DEFAULT_PAGE_V15545 = 10 if _PERF_LARGE_COHORT_V15545 else 20

if _PERF_LARGE_COHORT_V15545:
    st.sidebar.warning(
        f"⚡ Modo ultraligero automático: {_PERF_RECORD_COUNT_V15545} registros. "
        "El histórico completo sigue participando en los cálculos."
    )
else:
    st.sidebar.caption(f"Rendimiento: modo normal · {_PERF_RECORD_COUNT_V15545} registros")

st.session_state.setdefault("_perf_active_heavy_tab_v15545", None)

def _perf_heavy_tab_gate_v15545(tab_key, title):
    if not _PERF_LARGE_COHORT_V15545:
        return True
    active = st.session_state.get("_perf_active_heavy_tab_v15545")
    if active == tab_key:
        c1, c2 = st.columns([4, 1])
        with c1:
            st.success(f"⚡ {title} activa. Las demás pestañas pesadas permanecen suspendidas.")
        with c2:
            if st.button("Liberar", key=f"perf_release_{tab_key}", use_container_width=True):
                st.session_state["_perf_active_heavy_tab_v15545"] = None
                st.rerun()
        return True
    st.info(
        f"⚡ Cohorte grande ({_PERF_RECORD_COUNT_V15545} registros). "
        "Esta pestaña usa carga diferida para evitar reconstruir todas las gráficas en cada interacción."
    )
    if st.button(f"Cargar {title}", key=f"perf_load_{tab_key}", type="primary"):
        st.session_state["_perf_active_heavy_tab_v15545"] = tab_key
        st.rerun()
    return False

def _perf_paginated_dataframe_v15545(df, key, page_size=None, **kwargs):
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        st.dataframe(df if isinstance(df, pd.DataFrame) else pd.DataFrame(), **kwargs)
        return
    ps = int(page_size or _PERF_DEFAULT_PAGE_V15545)
    if len(df) <= ps:
        st.dataframe(df, **kwargs)
        return
    _, _, i0, i1 = _record_page_controls(
        len(df), f"perf_df_{key}", page_size=ps, label="filas"
    )
    st.caption(f"Mostrando filas {i0+1}–{i1} de {len(df)}.")
    st.dataframe(df.iloc[i0:i1], **kwargs)

def _perf_patient_frame_v15545(kind, patient_id):
    cache = st.session_state.get("_perf_patient_frames_v15545", {})
    return cache.get(kind, {}).get(normalize_patient_id(str(patient_id)), pd.DataFrame()).copy()
...
with tab3:
    if _perf_heavy_tab_gate_v15545("tab3", "Comparar"):
        ...
        if len(records) < 2:
            ...
            _perf_paginated_dataframe_v15545(valid_summary, "tab3_valid_summary", use_container_width=True)
...
with tab4:
    if _perf_heavy_tab_gate_v15545("tab4", "No lineales / MSE / Atractor"):
...
with tab5:
    if _perf_heavy_tab_gate_v15545("tab5", "Poincaré / Grafos"):
        ...
        if len(records) < 1:
            ...
            if not available_phases_pg:
                ...
                if modo_fases_pg != "Todas las fases del registro principal" and _has_multiple_patients(records):
                    ...
                    _srp, _srnp, _sr0, _sr1 = _record_page_controls(len(_single_records), "poincare_hvg_single_v1543", page_size=(8 if _PERF_LARGE_COHORT_V15545 else 12), label="registros")
                ...
                if not include_hvg:
                    ...
                    # v15.5.49 · El panel HVG multivariable deja de depender de una sola fase.
                    # Conservamos hvg_df para la tabla/red seleccionada, pero el panel puede usar
                    # todo el histórico disponible de registros y fases.
                    hvg_numeric = [
                        c for c in hvg_cols
                        if c in long_df.columns and pd.api.types.is_numeric_dtype(long_df[c])
                    ]
                    hvg_panel_base_df = (
                        long_df[["Registro", "Fase"] + hvg_numeric].copy()
                        if hvg_numeric and "Registro" in long_df.columns and "Fase" in long_df.columns
                        else pd.DataFrame()
                    )

                    if hvg_numeric and not hvg_panel_base_df.empty:
                        st.markdown("### Panel HVG multivariable")
                        st.caption(
                            "Compara la topología HVG entre fases y/o entre registros. "
                            "Las columnas siguen el orden cronológico de los registros y cada métrica conserva su propia escala."
                        )

                        # Modo propio del panel: no queda bloqueado por 'Fase para Poincaré / grafo'.
                        hvg_panel_mode_v15549 = st.radio(
                            "Comparación del panel HVG",
                            [
                                "Evolución de una fase entre registros",
                                "Basal · Ejercicio · Recuperación por registro",
                                "Fases concretas entre registros",
                                "Selección actual de Poincaré/grafos",
                            ],
                            horizontal=True,
                            key="hvg_panel_mode_v15549",
                            help=(
                                "Permite seguir, por ejemplo, todos los Basales a lo largo del tiempo, "
                                "comparar Basal/Ejercicio/Recuperación dentro de cada registro, "
                                "o elegir fases concretas como Basal, E1 y R1."
                            )
                        )

                        hvg_panel_df = pd.DataFrame()
                        hvg_panel_caption = ""

                        if hvg_panel_mode_v15549 == "Evolución de una fase entre registros":
                            hvg_phases_available_v15549 = [
                                p for p in PHASES
                                if p in set(hvg_panel_base_df["Fase"].dropna().astype(str))
                            ]
                            if not hvg_phases_available_v15549:
                                hvg_phases_available_v15549 = sorted(
                                    hvg_panel_base_df["Fase"].dropna().astype(str).unique().tolist()
                                )

                            _hvg_default_phase_v15549 = (
                                hvg_phases_available_v15549.index("Basal")
                                if "Basal" in hvg_phases_available_v15549 else 0
                            )
                            hvg_phase_long_v15549 = st.selectbox(
                                "Fase a seguir longitudinalmente",
                                hvg_phases_available_v15549,
                                index=_hvg_default_phase_v15549,
                                key="hvg_phase_long_v15549"
                            )
                            hvg_panel_df = hvg_panel_base_df[
                                hvg_panel_base_df["Fase"].astype(str) == str(hvg_phase_long_v15549)
                            ].copy()
                            hvg_panel_caption = (
                                f"Evolución longitudinal de **{hvg_phase_long_v15549}**: "
                                "una columna por registro, ordenada por fecha/hora. "
                                "Con varios registros puedes ver directamente cómo cambia la topología entre sesiones."
                            )

                        elif hvg_panel_mode_v15549 == "Basal · Ejercicio · Recuperación por registro":
                            hvg_panel_base_df["_Grupo_HVG_v15549"] = (
                                hvg_panel_base_df["Fase"].astype(str).map(PHASE_GROUP)
                            )
                            hvg_groups_available_v15549 = [
                                g for g in ["Basal", "Ejercicio", "Recuperación"]
                                if g in set(hvg_panel_base_df["_Grupo_HVG_v15549"].dropna().astype(str))
                            ]
                            hvg_groups_sel_v15549 = st.multiselect(
                                "Grupos fisiológicos",
                                ["Basal", "Ejercicio", "Recuperación"],
                                default=hvg_groups_available_v15549,
                                key="hvg_groups_sel_v15549",
                                help=(
                                    "Basal agrupa Basal/Basal2…; Ejercicio agrupa E1–E6; "
                                    "Recuperación agrupa R1–R6. Si hay varias subfases, el valor mostrado "
                                    "es la media de la métrica HVG dentro del grupo para ese registro."
                                )
                            )
                            _grp_src_v15549 = hvg_panel_base_df[
                                hvg_panel_base_df["_Grupo_HVG_v15549"].isin(hvg_groups_sel_v15549)
                            ].copy()
                            if not _grp_src_v15549.empty:
                                hvg_panel_df = (
                                    _grp_src_v15549
                                    .groupby(["Registro", "_Grupo_HVG_v15549"], as_index=False)[hvg_numeric]
                                    .mean(numeric_only=True)
                                    .rename(columns={"_Grupo_HVG_v15549": "Fase"})
                                )
                                hvg_panel_df["Fase"] = pd.Categorical(
                                    hvg_panel_df["Fase"],
                                    categories=["Basal", "Ejercicio", "Recuperación"],
                                    ordered=True
                                )
                                hvg_panel_df = hvg_panel_df.sort_values(["Fase", "Registro"])
                            hvg_panel_caption = (
                                "Cada registro puede mostrar **tres columnas fisiológicas**: Basal, Ejercicio y Recuperación. "
                                "Cuando un grupo contiene varias ventanas (E1–E6 o R1–R6), se representa su media topológica."
                            )

                        elif hvg_panel_mode_v15549 == "Fases concretas entre registros":
                            hvg_phases_available_v15549 = [
                                p for p in PHASES
                                if p in set(hvg_panel_base_df["Fase"].dropna().astype(str))
                            ]
                            _defaults_exact_v15549 = [
                                p for p in ["Basal", "E1", "R1"] if p in hvg_phases_available_v15549
                            ]
                            if not _defaults_exact_v15549:
                                _defaults_exact_v15549 = hvg_phases_available_v15549[:3]
                            hvg_exact_phases_v15549 = st.multiselect(
                                "Fases concretas",
                                hvg_phases_available_v15549,
                                default=_defaults_exact_v15549,
                                key="hvg_exact_phases_v15549"
                            )
                            hvg_panel_df = hvg_panel_base_df[
                                hvg_panel_base_df["Fase"].astype(str).isin(hvg_exact_phases_v15549)
                            ].copy()
                            hvg_panel_caption = (
                                "Las fases seleccionadas se muestran juntas para cada registro y mantienen "
                                "una tendencia longitudinal independiente."
                            )

                        else:
                            # Compatibilidad exacta con el comportamiento anterior.
                            hvg_panel_df = hvg_df.copy()
                            hvg_panel_caption = (
                                "Vista vinculada a la selección superior de Poincaré/grafos, "
                                "equivalente al comportamiento de versiones anteriores."
                            )

                        st.caption(hvg_panel_caption)
                        ...
                        hvg_multi_key = "hvg_vars_multiselect_v15549"
                        ...
                        st.session_state[hvg_multi_key] = [
                            c for c in st.session_state[hvg_multi_key] if c in hvg_numeric
                        ]

                        b1, b2, b3 = st.columns([1, 1.25, 1])
                        with b1:
                            if st.button("Seleccionar todas", key="hvg_select_all_v15549", use_container_width=True):
                                st.session_state[hvg_multi_key] = hvg_numeric[:]
                        with b2:
                            if st.button(
                                "Selección topológica recomendada",
                                key="hvg_select_recommended_v15549",
                                use_container_width=True
                            ):
                                st.session_state[hvg_multi_key] = recommended_hvg[:]
                        with b3:
                            if st.button("Limpiar", key="hvg_clear_v15549", use_container_width=True):
                                st.session_state[hvg_multi_key] = []
                        ...
                        if hvg_panel_df.empty:
                            st.info(
                                "No hay métricas HVG disponibles para esta combinación de fases/registros. "
                                "Comprueba las ventanas válidas y que HVG/grafos esté activado."
                            )
                        elif hvg_vars:
                            paginate_hvg = st.checkbox(
                                "Paginar las gráficas para mantener la pestaña ligera",
                                value=True,
                                key="hvg_paginate_v15549"
                            )
                            if paginate_hvg:
                                per_page = st.selectbox(
                                    "Gráficas HVG por página", [2, 4, 6, 8], index=1,
                                    key="hvg_plots_per_page_v15549"
                                )
                                n_pages = max(1, int(np.ceil(len(hvg_vars) / per_page)))
                                page = st.number_input(
                                    "Página HVG", min_value=1, max_value=n_pages, value=1, step=1,
                                    key="hvg_plot_page_v15549"
                                )
                                start_i = (int(page) - 1) * int(per_page)
                                shown_hvg_vars = hvg_vars[start_i:start_i + int(per_page)]
                                st.caption(
                                    f"Mostrando {start_i + 1}–{start_i + len(shown_hvg_vars)} "
                                    f"de {len(hvg_vars)} métricas · página {int(page)} de {n_pages}"
                                )
                            else:
                                shown_hvg_vars = hvg_vars

                            # Dos paneles por fila; cada eje mantiene su escala propia.
                            for i in range(0, len(shown_hvg_vars), 2):
                                ...
                                for j, hvg_var in enumerate(shown_hvg_vars[i:i+2]):
                                    with row_cols[j]:
                                        _hvg_plot_src_v15549 = hvg_panel_df[
                                            ["Registro", "Fase", hvg_var]
                                        ].copy()
                                        _hvg_plot_src_v15549[hvg_var] = pd.to_numeric(
                                            _hvg_plot_src_v15549[hvg_var], errors="coerce"
                                        )
                                        _hvg_plot_src_v15549 = _hvg_plot_src_v15549.dropna(subset=[hvg_var])

                                        if _hvg_plot_src_v15549.empty:
                                            st.info(f"{hvg_var}: sin datos válidos para la selección.")
                                            continue

                                        pivot_hvg = _hvg_plot_src_v15549.pivot_table(
                                            index="Fase",
                                            columns="Registro",
                                            values=hvg_var,
                                            aggfunc="first",
                                        ...
                                        st.plotly_chart(
                                            ...
                                            use_container_width=True,
                                            key=(
                                                f"hvg_compare_multi_{hvg_var}_"
                                                f"{hvg_panel_mode_v15549}_{phase_pg}_v15549"
                                            )
                                        )
                        else:
...
with tab6:
    if _perf_heavy_tab_gate_v15545("tab6", "Dashboard"):
...
with tab7:
    if _perf_heavy_tab_gate_v15545("tab7", "Informe"):
...
with tab9:
    if _perf_heavy_tab_gate_v15545("tab9", "Índices fisiológicos"):
        ...
        if _indices_active_v15534.empty:
            ...
            if view_mode == "Registro seleccionado":
                ...
                _perf_paginated_dataframe_v15545(_indices_active_v15534[[c for c in show_cols if c in _indices_active_v15534.columns]], "tab9_indices", use_container_width=True)
...
with tab10:
    if _perf_heavy_tab_gate_v15545("tab10", "Control autonómico"):
        st.subheader("Control autonómico · Diana V/S + fuzzy adaptativo + desacoplamiento fisiológico · Performance Large Cohorts")
        st.info(
        ...
        if _auto_table_v15521.empty:
            ...
            with st.container(border=True):
                ...
                _dual_fig_v15543 = _autonomic_dual_dashboard_v15543(
                    ...
                    dynamic_class=(str(_ar.get('Caracterizacion_desacoplamiento',''))
                                   if bool(_ar.get('Desacoplamiento_permitido', False)) else ""),
                    iid=pd.to_numeric(_ar.get('IID', np.nan), errors='coerce')
                )
                st.plotly_chart(
                    _dual_fig_v15543,
                    use_container_width=True,
                ...
                if bool(_ar.get('Desacoplamiento_permitido', False)):
                    st.markdown("### Caracterización dinámica del desacoplamiento")
                    _iid=pd.to_numeric(_ar.get('IID',np.nan),errors='coerce')
                    _iidc=pd.to_numeric(_ar.get('IID_confianza',np.nan),errors='coerce')
                    _dc=str(_ar.get('Caracterizacion_desacoplamiento','Indeterminado'))
                    _d1,_d2,_d3=st.columns(3)
                    _d1.metric("Integridad dinámica (IID)",f"{_iid:.0f}/100" if np.isfinite(_iid) else "N/D")
                    _d2.metric("Caracterización",_dc)
                    _d3.metric("Confianza dinámica",f"{_iidc:.0f}%" if np.isfinite(_iidc) else "N/D")
                    st.write(str(_ar.get('Detalle_caracterizacion_desacoplamiento','')))
                    st.dataframe(pd.DataFrame({
                        "Dominio":["Fractalidad","Entropía multiescala","Atractor/geometría","Recurrencia","HVG/red"],
                        "Integridad (0-100)":[_ar.get('IID_Fractalidad',np.nan),_ar.get('IID_Entropia_multiescala',np.nan),
                        _ar.get('IID_Atractor_geometria',np.nan),_ar.get('IID_Recurrencia',np.nan),_ar.get('IID_HVG_red',np.nan)]
                    }),use_container_width=True,hide_index=True)
                    st.caption("El IID caracteriza la organización dinámica del desacoplamiento; no diagnostica patología. "
                               "'Compatible con pérdida de complejidad' requiere correlación clínica.")
                ...
                with st.expander("Cómo se decide la topología actual", expanded=False):
                    ...
                    _top_scores_v15530 = pd.DataFrame({
                        "Familia": [
                            ...
                            "Patrón preservado con tendencia a coinhibición",
                        ...
                        "Permitida": [
                            ...
                            bool(_ar.get('Tendencia_Coinhibicion_permitida', False)),
                        ...
                        "Score": [
                            ...
                            _ar.get('Score_Tendencia_Coinhibicion_actual', np.nan),
                        ...
                        "Score_bruto": [
                            ...
                            _ar.get('Score_Tendencia_Coinhibicion_bruto_actual', np.nan),
                    ...
                    _dp1_v15547, _dp2_v15547 = st.columns(2)
                    with _dp1_v15547:
                        _pdmean_v15547 = pd.to_numeric(_ar.get('Profundidad_negativa_media_VS', np.nan), errors='coerce')
                        st.metric("Profundidad negativa media V/S", f"{_pdmean_v15547:.2f}" if np.isfinite(_pdmean_v15547) else "N/D")
                    with _dp2_v15547:
                        _pdmax_v15547 = pd.to_numeric(_ar.get('Profundidad_negativa_max_VS', np.nan), errors='coerce')
                        st.metric("Profundidad negativa máxima V/S", f"{_pdmax_v15547:.2f}" if np.isfinite(_pdmax_v15547) else "N/D")
                    st.caption(
                    ...
                    st.caption(
                        "La v15.5.47 usa un gradiente continuo de profundidad negativa respecto a la zona gris. "
                        "Dos ramas V/S negativas con desplazamiento conjunto suficiente se clasifican como "
                        "'Patrón preservado con tendencia a coinhibición', aunque sigan dentro de la zona preservada. "
                        "Valores negativos mínimos próximos a cero no activan esta categoría. La coinhibición franca "
                        "sigue exigiendo reducción conjunta suficiente y salida real de la zona preservada."
                    )
                    ...
                    st.caption(
                        "El desacoplamiento fisiológico exige control unilateral: una rama V/S se desplaza "
                        "mientras la otra permanece próxima a la zona neutra/preservada. La discordancia metodológica "
                        "se mantiene como patrón complejo/incoherente. Si hay desacoplamiento, el IID caracteriza "
                        "si la dinámica es adaptativa/funcional, compatible con pérdida de complejidad o indeterminada."
                    )
            ...
            with st.expander("Historial cronológico completo", expanded=False):
                ...
                _perf_paginated_dataframe_v15545(_show_auto_v15521, "tab10_control_full", use_container_width=True, hide_index=True)

                st.download_button(
                    "Descargar Control Autonómico CSV",
                    _auto_table_v15521.to_csv(index=False).encode('utf-8'),
                    file_name='control_autonomico_v15549.csv',
                    mime='text/csv'
                )

with tab11:
    if _perf_heavy_tab_gate_v15545("tab11", "Predicción fisiológica"):
...
with tab8:
    if _perf_heavy_tab_gate_v15545("tab8", "Exportar"):
